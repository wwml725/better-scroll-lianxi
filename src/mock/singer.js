...
//创建一个函数，是得传入不同的参数，得到不同的对象
function Singer({name, id}) {
  //函数执行并且传入实参{{name: 1, id: 2}}相当于let {name,id} = {name: 1, id: 2}
  //这就是es6的对象的解构赋值
...
//把一个对象作为参数，就是将对象的解构赋值结合在了一起
//  console.log(new Singer({name: 1, id: 2}));
//数据序列化
var _normalizeData = (list) => {
  //首先创建一个热门
  let map = {
    hot: {
      title: "热门",
  ...
  //1、给hot添加数据map.hot.items.push(),  2、添加什么数据   3、获取data中的前十条数据
  list.map((item, index) => {
    if (index < 10) {
      map.hot.items.push(
        new Singer({
    ...
    //在map对象中添加其他项，比如A:{title:"A",items:[]}...
  ...
  for (let key in map) {
    ...
    res = res.filter((item, index) => {
      return item.title.match(/[a-zA-Z]/)
    })
...
//数组进行排序
 ...
 const singer = _normalizeData(data.list)

console.log(singer);
 
 export default singer;

// console.log(_normalizeData(data.list));
// console.log(_normalizeData(data.list)[0].title);


//想办法另每一项，都添加一个图片地址，地址的大部分代码是相同的，只是有一部分代码可以代表不同的图片（这一部分就是id），应该怎么写呢？
